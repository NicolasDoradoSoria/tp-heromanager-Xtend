...
class RepoSuperIndividuo extends Repositorio<SuperIndividuo> {
	...
	def elementosQueNoEstanEnLaLista(List<SuperIndividuo> individuos) {
		...
		elementos.filter[a|!individuos.exists[b|a.id==b.id]].toList}
	}
	...
	def agregarCompra(Compra compra,String id) {
		val individuo = searchById(id)
		individuo.comprarItem(compra)
		update(individuo) 
	}
...
class RepoItem extends Repositorio<Item> {
	...
	def itemsEnVenta(){
		elementos.filter(item|item.stock>0).toList
	}
	
	def descontarStock(Item item, int cantidad) {
		val itemToUpdate = searchById(item.id)
		itemToUpdate.descontarStock(cantidad)
