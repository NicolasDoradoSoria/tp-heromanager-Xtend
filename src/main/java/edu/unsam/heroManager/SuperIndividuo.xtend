...
class SuperIndividuo implements Entidad {
	...
	def agregarEnemigo(SuperIndividuo enemigo) {
		if (enemigo == this) {
			throw new RuntimeException("No puede ser enemigo de si mismo")
		}
	...
	def comprasUltimaSemana() {
		compras.filter(compra|compra.esDeLaUltimaSemana).toList
	}

	def comprarItem(Compra compra) {
		if (dinero < compra.precioTotal) {
			throw new Exception("Cantidad de dinero insuficiente")
		}
